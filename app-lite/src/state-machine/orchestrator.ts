...
async function getSystemConfig(db: DbClient) {
  ...
  return {
    current_phase: config.currentPhase ?? config.current_phase ?? 2,
    email_approval_required: config.emailApprovalRequired ?? config.email_approval_required ?? true,
    demo_approval_required: config.demoApprovalRequired ?? config.demo_approval_required ?? true,
    launch_approval_required: config.launchApprovalRequired ?? config.launch_approval_required ?? true,
    active_niche: config.activeNiche ?? config.active_niche ?? '',
    active_cities: config.activeCities ?? config.active_cities ?? [],
    dodo_checkout_links: config.dodoCheckoutLinks ?? config.dodo_checkout_links ?? {},
    maintenance_mode: config.maintenanceMode ?? config.maintenance_mode ?? false,
  }
}
