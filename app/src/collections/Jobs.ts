...
export const Jobs: CollectionConfig = {
  slug: 'jobs',
  admin: { group: 'System', defaultColumns: ['job_type', 'status', 'run_at', 'attempts', 'createdAt'] },
  fields: [
    { name: 'id', type: 'text', required: true, defaultValue: () => crypto.randomUUID() },
    { name: 'job_type', type: 'select', required: true, options: ['lead_gen', 'follow_up_1', 'follow_up_2', 'demo_build', 'onboarding', 'monthly_report', 'churn_check', 'support_auto_reply', 'billing_retry', 'site_qa'] },
    { name: 'lead', type: 'relationship', relationTo: 'leads' },
    { name: 'client', type: 'relationship', relationTo: 'clients' },
    { name: 'status', type: 'select', required: true, defaultValue: 'queued', options: ['queued', 'running', 'completed', 'failed', 'dead'] },
