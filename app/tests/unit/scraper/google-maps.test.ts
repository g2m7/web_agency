...
describe('parseBusinessListings', () => {
  it('extracts businesses from mock Google Maps HTML', () => {
    ...
    expect(johns!.name).toContain('HVAC')
  ...
  it('deduplicates businesses by name', () => {
    const dupHtml = `
    "0xaaa:0xbbb", [null,"Same Business Name","addr","(512) 111-2222"],
    "0xccc:0xddd", [null,"Same Business Name","addr2","(512) 333-4444"],
    `
  ...
  it('returns empty array for empty HTML', () => {
    const results = parseBusinessListings('', 'Austin', 'TX')
    expect(results).toEqual([])
  })

  it('returns empty array for HTML without place IDs', () => {
    const results = parseBusinessListings('<html><body>Hello</body></html>', 'Austin', 'TX')
    expect(results).toEqual([])
  })
...
describe('buildSearchUrl', () => {
  it('builds URL with encoded query', () => {
...
describe('parseCityState', () => {
  ...
  it('parses "San Antonio, TX" format', () => {
    const { city, state } = parseCityState('San Antonio, TX')
    expect(city).toBe('San Antonio')
    expect(state).toBe('TX')
  })

  it('parses "New York NY" space-separated format', () => {
...
describe('buildNicheCityKey', () => {
  it('builds lowercase colon-separated key', () => {
    const key = buildNicheCityKey('HVAC', 'Austin', "John's HVAC")
    expect(key).toBe('hvac:austin:john\'s hvac')
  })

  it('normalizes whitespace in business name', () => {
    const key = buildNicheCityKey('hvac', 'Austin', 'Cool  Air   Austin')
    expect(key).toBe('hvac:austin:cool air austin')
  })
})
